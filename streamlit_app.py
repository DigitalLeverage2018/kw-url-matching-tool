...
content_file = st.file_uploader("content_upload", type="csv", label_visibility="collapsed")
...
keywords_file = st.file_uploader("keywords_upload", type="csv", label_visibility="collapsed")
...
progress_text_kw = st.empty()
progress_kw = st.progress(0)
st.subheader("📄 1. Beste URLs pro Keyword")
rows_kw_url = []
total_kw = len(df_keywords)
total_kw = len(df_keywords)
for i, kw in enumerate(df_keywords["keyword"]):
    progress_text_kw.text(f"🔍 Keyword {i + 1} von {total_kw}: {kw}")
    progress_kw.progress((i + 1) / total_kw)
...
progress_text_url = st.empty()
progress_url = st.progress(0)
st.subheader("📄 2. Beste Keywords pro URL")
rows_url_kw = []
total_urls = len(df_content)
total_urls = len(df_content)
for j, page_url in enumerate(df_content["url"]):
    progress_text_url.text(f"🔍 URL {j + 1} von {total_urls}: {page_url}")
    progress_url.progress((j + 1) / total_urls)
